...
std::string BenchmarkStats::ToString() const {
  ...
  oss << "Latency (us): min=" << latency_min_us 
      << " avg=" << latency_avg_us
      << " p50=" << latency_p50_us 
      << " p99=" << latency_p99_us
      << " max=" << latency_max_us << "\n";
...
BenchmarkStats Benchmark::Run() {
  ...
  stats.success_rate = (op_count > 0) ? 
      static_cast<double>(success_count) / op_count : 0.0;
  stats.duration_ms = duration;
  stats.operations_per_second = (duration.count() > 0) ? 
      (static_cast<double>(op_count) * 1000.0) / duration.count() : 0.0;
...
OperationResult ThroughputBenchmark::DoOperation() {
  return operation_();
}
...
OperationResult LatencyBenchmark::DoOperation() {
  return operation_();
}
...
FailoverBenchmark::FailoverBenchmark(
    std::function<OperationResult()> operation,
...
OperationResult FailoverBenchmark::DoOperation() {
  return operation_();
}
...
void PrintResults(const std::vector<BenchmarkStats>& results) {
  ...
  for (size_t i = 0; i < results.size(); ++i) {
    ...
    std::cout << std::left << std::setw(10) << (i + 1)
              << std::setw(15) << static_cast<int>(s.operations_per_second)
              << std::setw(12) << std::fixed << std::setprecision(1) << (s.success_rate * 100)
