...
int main(int argc, char* argv[]) {
  ...
  std::cout << std::endl;
  std::cout << "Type: " << (config.operation_type == BenchmarkConfig::OperationType::kWrite ? "write" :
                           config.operation_type == BenchmarkConfig::OperationType::kRead ? "read" : "mixed") << std::endl;
  ...
  if (config.num_clients == 1) {
    ...
    stats.success_rate = (stats.total_operations > 0) ? 
        static_cast<double>(stats.success_count) / stats.total_operations : 0.0;
