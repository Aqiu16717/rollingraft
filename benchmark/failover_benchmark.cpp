...
void PrintUsage(const char* program) {
  std::cout << "Usage: " << program << " [options] <server1> <server2> ...\n"
            ...
            << "  -c, --check <command>    Shell command to check if new leader elected\n"
...
FailoverResult RunFailoverBenchmark(
    Client* client,
  ...
  while (std::chrono::duration_cast<std::chrono::seconds>(
             std::chrono::steady_clock::now() - warmup_start).count() < warmup_seconds) {
  ...
  while (std::chrono::steady_clock::now() - kill_time < max_wait) {
    ...
    if (!check_command.empty() && ExecuteCommand(check_command) == 0) {
      if (!recovered) {
        ...
        result.detection_time = std::chrono::duration_cast<std::chrono::milliseconds>(
            detection_time - kill_time);
        result.election_time = std::chrono::duration_cast<std::chrono::milliseconds>(
      ...
      if (!detected) {
        ...
        std::cout << "\nFailure detected after " 
                  << std::chrono::duration_cast<std::chrono::milliseconds>(
                      detection_time - kill_time).count() << " ms" << std::endl;
      }
  ...
  result.availability_during_failover = (failover_ops > 0) ?
      static_cast<double>(failover_ops - failover_failed) / failover_ops : 0.0;
...
int main(int argc, char* argv[]) {
  ...
  auto result = RunFailoverBenchmark(
      &client, kill_command, check_command, ops_per_second, warmup_seconds);
  
  // Print results
  std::cout << "\n========== Failover Results ==========" << std::endl;
  std::cout << std::left << std::setw(30) << "Metric" << "Value" << std::endl;
  std::cout << std::string(50, '-') << std::endl;
  std::cout << std::left << std::setw(30) << "Failure Detection Time:"
            << result.detection_time.count() << " ms" << std::endl;
  std::cout << std::left << std::setw(30) << "Leader Election Time:"
            << result.election_time.count() << " ms" << std::endl;
  std::cout << std::left << std::setw(30) << "Total Recovery Time:"
            << result.recovery_time.count() << " ms" << std::endl;
  std::cout << std::left << std::setw(30) << "Operations During Failover:"
            << result.operations_during_failover << std::endl;
  std::cout << std::left << std::setw(30) << "Failed Operations:"
            << result.operations_failed << std::endl;
  std::cout << std::left << std::setw(30) << "Availability During Failover:"
            << std::fixed << std::setprecision(1)
            << (result.availability_during_failover * 100.0) << "%" << std::endl;
