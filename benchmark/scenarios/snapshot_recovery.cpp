...
class SnapshotRecoveryScenario : public ClusterBenchmark {
 ...
 public:
  BenchmarkStats Run() override {
    ...
    std::cout << "\n=== Phase 1: Measure latency during snapshot ==="
              << std::endl;
    ...
    std::cout << "Phase 1 complete. Max P99 latency: " << max_p99 << " us"
              << std::endl;

    // Phase 2: Stop a follower, restart it, measure catch-up time
    std::cout << "\n=== Phase 2: Follower restart and catch-up ==="
              << std::endl;
    ...
    std::cout << "Stopping follower node " << (follower_idx + 1) << "..."
              << std::endl;
    StopNode(follower_idx);

    // Let some new entries accumulate while follower is down
    std::cout << "Generating new entries while follower is down..."
              << std::endl;
    for (int i = 0; i < 500; ++i) {
    ...
    while (std::chrono::steady_clock::now() - catch_up_start <
           max_catch_up_wait) {
...
REGISTER_SCENARIO(snapshot_recovery, []() {
  return std::make_unique<SnapshotRecoveryScenario>();
});
