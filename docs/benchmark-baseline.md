...
```bash
# 默认参数（leveldb, 50000 entries, 100B payload, batch=[1,10,100], compression=[0,1]）
...
# HybridPersister 就绪后
./benchmark/benchmark_persister --backend=hybrid --output=hybrid_results.csv
```

### 批量跑全矩阵并生成报告

```bash
./scripts/run_persister_benchmark.sh build_test benchmark/results
```

输出：
- `benchmark/results/persister_benchmark_YYYYMMDD_HHMMSS.csv` — 原始数据
- `benchmark/results/persister_benchmark_YYYYMMDD_HHMMSS.md` — 汇总报告
...
*Framework extended for T3 Phase 3 comparison by @GeoHot.*
