...
void RaftNode::RaftNodeImpl::ApplyConfigChangeLocked(const std::string& cmd) {
  ...
  // Malformed commands (corrupted log, buggy peer) must not throw out of the
  // apply thread — std::stoll and nlohmann::json::parse both throw on bad
  // input, and an escaping exception terminates the process.
  try {
    ...
    if (cmd.starts_with("CONFIG_CHANGE:PROMOTE:")) {
      ...
      if (!group_->cluster_config_.IsVoter(id)) {
        ...
        LOG_INFO("Node {} applied PromoteLearner for {} (config version {})", group_->server_id_,
                 id, group_->cluster_config_.version);
        PersistClusterConfigLocked();
      }
      return;
    }
    ...
    if (cmd.starts_with("CONFIG_CHANGE:REMOVE:")) {
      ...
      group_->cluster_config_.nodes.erase(std::remove(group_->cluster_config_.nodes.begin(),
                                                      group_->cluster_config_.nodes.end(), id),
    ...
    LOG_ERROR("Node {} ignoring malformed config change command '{}': {}", group_->server_id_, cmd,
              e.what());
  }
}
