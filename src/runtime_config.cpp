...
Status RuntimeConfig::UpdateFromJson(const std::string& json_str) {
  ...
  if (j.contains("heartbeat_interval_ms")) proposed.heartbeat_interval_ms = j["heartbeat_interval_ms"];
  if (j.contains("max_entries_per_append")) proposed.max_entries_per_append = j["max_entries_per_append"];
  if (j.contains("rpc_timeout_ms")) proposed.rpc_timeout_ms = j["rpc_timeout_ms"];
  if (j.contains("snapshot_threshold_entries")) proposed.snapshot_threshold_entries = j["snapshot_threshold_entries"];
  if (j.contains("snapshot_threshold_bytes")) proposed.snapshot_threshold_bytes = j["snapshot_threshold_bytes"];
  if (j.contains("snapshot_check_interval_ms")) proposed.snapshot_check_interval_ms = j["snapshot_check_interval_ms"];
...
Status RuntimeConfig::Validate(const Values& v) const {
  ...
  if (v.snapshot_threshold_bytes < 1 * 1024 * 1024 || v.snapshot_threshold_bytes > 1 * 1024 * 1024 * 1024) {
