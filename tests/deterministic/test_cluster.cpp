...
TestCluster::~TestCluster() { StopAll(); for (const auto& d : data_dirs_) std::filesystem::remove_all(d); }
void TestCluster::StartAll() { for (size_t i = 0; i < options_.num_nodes; ++i) StartNode(static_cast<NodeId>(i)); }
void TestCluster::StopAll() { for (size_t i = 0; i < options_.num_nodes; ++i) StopNode(static_cast<NodeId>(i)); }
...
void TestCluster::CrashNode(NodeId id) { if (static_cast<size_t>(id) >= options_.num_nodes) return; nodes_[id].reset(); }
...
void TestCluster::AssertCommitted(Index index) const { for (size_t i = 0; i < options_.num_nodes; ++i) if (nodes_[i]) ASSERT_GE(GetCommitIndex(static_cast<NodeId>(i)), index); }
void TestCluster::AssertAllApplied(Index index) const { for (size_t i = 0; i < options_.num_nodes; ++i) if (nodes_[i]) ASSERT_GE(GetLastApplied(static_cast<NodeId>(i)), index); }
void TestCluster::AssertStateMachineEqual() const { std::vector<std::string> ref; for (size_t i = 0; i < options_.num_nodes; ++i) { if (!nodes_[i]) continue; auto c = state_machines_[i]->GetAppliedCommands(); if (ref.empty()) ref = c; else ASSERT_EQ(c, ref); } }
...
MockStateMachine* TestCluster::GetStateMachine(NodeId id) const { if (static_cast<size_t>(id) >= options_.num_nodes) return nullptr; return state_machines_[id].get(); }
