...
void TestCluster::CrashNode(NodeId id) {
  if (static_cast<size_t>(id) >= options_.num_nodes) {
    return;
  }
  nodes_[id].reset();
}
...
Status TestCluster::ProposeToLeader(const std::string& command) {
  ...
  if (lid == -1) {
    return Status::Error("NO_LEADER", "No leader");
  }
...
void TestCluster::AssertCommitted(Index index) const {
  for (size_t i = 0; i < options_.num_nodes; ++i) {
    if (nodes_[i]) {
      ASSERT_GE(GetCommitIndex(static_cast<NodeId>(i)), index);
    }
  }
}
void TestCluster::AssertAllApplied(Index index) const {
  for (size_t i = 0; i < options_.num_nodes; ++i) {
    if (nodes_[i]) {
      ASSERT_GE(GetLastApplied(static_cast<NodeId>(i)), index);
    }
  }
}
...
std::vector<NodeId> TestCluster::GetLeaderIds() const {
  ...
  for (size_t i = 0; i < options_.num_nodes; ++i) {
    if (nodes_[i] && nodes_[i]->IsLeader()) {
      l.push_back(static_cast<NodeId>(i));
    }
  }
  return l;
}
...
Index TestCluster::GetLastApplied(NodeId id) const {
  if (static_cast<size_t>(id) >= options_.num_nodes || !state_machines_[id]) {
    return 0;
  }
