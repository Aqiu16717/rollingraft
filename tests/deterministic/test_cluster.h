...
class TestCluster {
 public:
  ...
  void AssertAllApplied(Index index) const;
  void AssertStateMachineEqual() const;
  RaftNodeRole GetRole(NodeId id) const;
  NodeId GetLeaderId() const;
  std::vector<NodeId> GetLeaderIds() const;
  Index GetCommitIndex(NodeId id) const;
  Index GetLastApplied(NodeId id) const;
  size_t GetRunningNodeCount() const;
  SimulatedClock* GetClock() const;
  SimulatedNetwork* GetNetwork() const;
  RaftNode* GetNode(NodeId id) const;
  MockStateMachine* GetStateMachine(NodeId id) const;
