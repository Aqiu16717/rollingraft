...
class TestableMetricsHttpServer : public MetricsHttpServer {
 public:
  TestableMetricsHttpServer(const std::string& bind_addr,
...
TEST_F(MetricsHttpServerAuthTest, NoTokenAllowsAll) {
  ...
  auto [body, status, ct, sse] =
      server.TestBuildResponse(MakeRequest("POST", "/v1/members"));
...
TEST_F(MetricsHttpServerAuthTest, MissingTokenReturns401) {
  ...
  EXPECT_TRUE(IsUnauthorized(status))
      << "Expected 401 for missing token, got: " << status;
...
TEST_F(MetricsHttpServerAuthTest, WrongTokenReturns401) {
  ...
  auto [body, status, ct, sse] = server.TestBuildResponse(
      MakeRequest("POST", "/v1/members", "", "wrong-token"));
  EXPECT_TRUE(IsUnauthorized(status))
      << "Expected 401 for wrong token, got: " << status;
}
...
TEST_F(MetricsHttpServerAuthTest, CorrectTokenAllowsAdmin) {
  ...
  auto [body, status, ct, sse] = server.TestBuildResponse(
      MakeRequest("POST", "/v1/members", "", "secret-token"));
  EXPECT_TRUE(IsSuccess(status) || status.find("404") != std::string::npos)
...
TEST_F(MetricsHttpServerAuthTest, PublicEndpointsIgnoreToken) {
  ...
  auto [body1, status1, ct1, sse1] =
      server.TestBuildResponse(MakeRequest("GET", "/metrics"));
  EXPECT_TRUE(IsSuccess(status1))
      << "Expected 200 for /metrics, got: " << status1;

  // /healthz should be accessible without token
  auto [body2, status2, ct2, sse2] =
      server.TestBuildResponse(MakeRequest("GET", "/healthz"));
  EXPECT_TRUE(IsSuccess(status2))
      << "Expected 200 for /healthz, got: " << status2;

  // /v1/status should be accessible without token
  auto [body3, status3, ct3, sse3] =
      server.TestBuildResponse(MakeRequest("GET", "/v1/status"));
...
TEST_F(MetricsHttpServerAuthTest, AllAdminEndpointsProtected) {
  ...
  for (const auto& ep : admin_endpoints) {
    auto [body, status, ct, sse] =
        server.TestBuildResponse(MakeRequest(ep.method, ep.path));
    EXPECT_TRUE(IsUnauthorized(status))
        << "Expected 401 for " << ep.method << " " << ep.path
        << ", got: " << status;
  }
}
